...
?>
<h2 id="center">シューズ</h2>
<div class="product">
    <?php
    require_once 'DB_Manager.php';
    try{
        $pdo=getDB();
        $sql=$pdo->prepare('select image,item_name,price,category_id,inventory from m_items where item_code IN(134,141,148,155)');
        $sql->execute();
        ...
        foreach ($resultList as $item) {
            ...
            echo '<p id="white">￥',$item['price'],'(税込)/1足</p>';
            if ($item['inventory']>0) {
                echo '<div class="num">';
