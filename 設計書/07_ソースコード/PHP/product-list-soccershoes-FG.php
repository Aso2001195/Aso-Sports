...
?>
    <h2 id="center">スパイク（FG）</h2>
    <div class="product">
<?php
require_once 'DB_Manager.php';
try{
    $pdo=getDB();
    $sql=$pdo->prepare('select image,item_name,price,category_id,inventory from m_items where item_code IN(30,37,44,51)');
    $sql->execute();
    ...
    foreach ($resultList as $item) {
        ...
        echo '</a>';
        echo '<p id="white">サイズ:<select name="size',$num,'">';
        for($j=240;$j<=270;$j+=5){
            echo '<option value = "',$j,'">',$j/10,'cm</option>';
        }
        echo '</select></p>';
        echo '<p id="white">￥',$item['price'],'(税込)/1本</p>';
        if($item['inventory']>0) {
            echo '<div class="num">';
